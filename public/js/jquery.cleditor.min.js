...
c=a.split("");if(b)for(a=(b[1]<<16|b[2]<<8|b[3]).toString(16);a.length<6;)a="0"+a;return"#"+(a.length==6?a:c[1]+c[1]+c[2]+c[2]+c[3]+c[3])}function r(){e.each(p,function(a,b){e(b).hide().unbind(q).removeData(A)})}function U(){var a=e("link[href$='jquery.cleditor.css']").attr("href");return a.substr(0,a.length-19)+"images/"}function K(a){var b=a.$main,c=a.options;a.$frame&&a.$frame.remove();var d=a.$frame=e('<iframe class="iframeclass class'+inctemp+'" frameborder="0" src="javascript:true;">').hide().appendTo(b),h=d[0].contentWindow,f=
...
imagesPath:function(){return U()}};e.fn.cleditor=function(a){var b=e([]);this.each(function(c,d){if(d.tagName=="TEXTAREA"){var h=e.data(d,Y);

/*	var tmpiframe = d.id;
alert(jQuery('iframe .class'+inctemp).contents().html());

jQuery('iframe .class'+inctemp).contents().find('body').attr('id', tmpiframe); 
inctemp++;	*/
